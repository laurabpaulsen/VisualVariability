...
def plot_hist_of_corr(ax, corr, bins, color="lightgreen", y_lim=(-0.5, 0.5)):
...
def plot_corr_hist(acc, save_path = None, corr_color="green", perm_color="#aed580", alpha=0.05, cluster=False, n_perm=1000, correlation=pearsonr, distance = "days", stratified_permutation=False):
    ...
    for perm in all_perm:
        ax_corr.plot(perm, color=perm_color, linewidth=0.5, alpha=0.2)
...
if __name__ == "__main__":
    ...
    acc = np.load(path.parents[0] / "accuracies" / "cross_decoding_10_LDA_sens.npy", allow_pickle=True)
