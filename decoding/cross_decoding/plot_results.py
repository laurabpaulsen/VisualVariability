...
def plot_cross_decoding_matrix(acc, save_path = None):
    ...
    for i in range(acc.shape[0]):
        for j in range(acc.shape[1]):
            ...
            axs[i, j].plot(np.diagonal(acc[i, j]), label="Accuracy")

            #plot.plot_tgm_ax(acc[i, j], ax=axs[i, j], vmin=35, vmax=65)
            # add diagonal line
            #add_diagonal_line(axs[i, j], colour='grey', linewidth=2)

            axs[i, j].axhline(0.5, linestyle ="--", color = "grey", linewidth = 2, label = "50% accuracy")
            # remove x ticks
            axs[i, j].set_xticks([])
            axs[i, j].set_yticks([])
            axs[i, j].set_ylim(0.40, 0.70)
...
def average_diagonal(acc, alpha = None, acc_pair = None, save_path = None):
    ...
    for t, v in zip(time_peaks, value_peaks):

        # get the standard deviation of the peaks from the acc pair:
        if acc_pair is not None:
            diagonals_acc_pair = np.array([np.diagonal(acc_pair[i, j], axis1=0, axis2=1) for i in range(acc_pair.shape[0]) for j in range(acc_pair.shape[1]) if i != j])
            print(diagonals_acc_pair[:, int(t*250)].shape)
            peaks_values = diagonals_acc_pair[:, int(t*250)]*100
            std = np.std(peaks_values)

        print(f"Peak at {t}s with value {v}%, std = {std}%")

    if alpha:
        # detect where significance is reached
        sig = np.where(diag > chance_level(588, alpha = alpha, p = 0.5)*100)[0]
        # plot a dashed line with chance level
        cl = chance_level(588, alpha = alpha, p = 0.5)*100
        print(f"Chance level: {cl}")
...
def average_diagonal_within(acc, alpha = None, acc_pair = None, save_path = None):
    ...
    for t, v in zip(time_peaks, value_peaks):
       if acc_pair is not None:
            # get the standard deviation of the peaks from the acc pair:
            diagonals_acc_pair = np.array([np.diagonal(acc_pair[:, :, i], axis1=0, axis2=1) for i in range(acc_pair.shape[2])])
            print(diagonals_acc_pair[:, int(t*250)].shape)
            peaks_values = diagonals_acc_pair[:, int(t*250)]*100
            std = np.std(peaks_values)
            print(f"Peak at {t}s with value {v}%, std = {std}%")
