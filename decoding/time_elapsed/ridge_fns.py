...
def diagonal_ridge_scores(X, y, cv = 10, alphas = np.logspace(0, 2, 10), ncv = 10, logger = None, return_betas = False):
    """
    ...
    predictions = np.empty((n_timepoints, cv, n_trials))
    true_values = np.empty((n_timepoints, cv, n_trials))
    ...
    for i in range(n_timepoints):
        ...
        for s, (train_idx, test_idx) in enumerate(folds.split(X[i, :, :])):
            ...
            predictions[i, s, :pred.shape[0]] = pred
            true_values[i, s, :pred.shape[0]] = y_test
